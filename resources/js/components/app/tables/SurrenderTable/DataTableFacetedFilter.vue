...
function filterFunction(
    ...
    if (typeof val[0] === 'string') {
        return (val as string[]).filter(item => item.toLowerCase().includes(term.toLowerCase()))
    }

    if (typeof val[0] === 'number') {
        return (val as number[]).filter(item => item.toString().includes(term))
    }

    if (typeof val[0] === 'object' && val[0] !== null) {
        return (val as Record<string, any>[]).filter(item =>
