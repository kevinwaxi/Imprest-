    ...
    <DialogScrollContent class="sm:min-w-4xl">
      ...
      <form class="space-y-6" @submit.prevent="submit">
          ...
          <CardContent>
            <div class="grid grid-cols-2 gap-4">
              <div class="grid grid-cols-2 gap-2 col-span-2">
                <!-- Account -->
                <Select v-model="commonFields.account_id" class="w-full" default-value="1">
                  <SelectTrigger class="**:data-desc:hidden w-auto max-w-full min-w-48">
                    <SelectValue placeholder="Choose account" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem
                      v-for="option in allAccounts" :key="option.value"
                      :value="option.value"
                    >
                ...
                <Select v-model="commonFields.project_id" class="w-full">
                  <SelectTrigger class="**:data-desc:hidden w-auto max-w-full min-w-48">
                    <SelectValue placeholder="Choose project" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem
                      v-for="option in allProjects" :key="option.value"
                      :value="option.value"
                    >
                      <SelectItemText>{{ option.label }}</SelectItemText>
                    </SelectItem>
              ...
              <!-- Amount -->
              <Input
                v-model="commonFields.amount" placeholder="Amount" type="number"
          ...
          <Card v-for="(warrant, index) in form.warrants" :key="index" class="relative">
            <CardHeader>
              <div class="flex justify-between items-center">
                ...
                <Button
                  v-if="form.warrants.length > 1" type="button" variant="destructive" size="icon"
            ...
            <CardContent>
              <div class="grid grid-cols-3 gap-4">
                ...
                <Select v-model="warrant.staff_id" class="w-full">
                  ...
                  <SelectContent>
                    <SelectItem
                      v-for="option in allStaff" :key="option.value"
                      :value="option.value"
                    >
                      <SelectItemText>{{ option.label }}</SelectItemText>
                    </SelectItem>
        ...
        <div class="flex gap-3">
          <Button
            type="button" variant="outline" :disabled="form.warrants.length >= maxWarrants"
            @click="addWarrant"
          >
